...
func (as *stockStorage) GetFromTo(ctx context.Context, sku uint64, dateFrom time.Time, dateTo time.Time) ([]*pb.Stock, error) {
	var result []*pb.Stock

	rows, err := as.client.Query(ctx, selectQuery, sku, dateFrom, dateTo)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
